...
const GridData = ({
  ...
  const userData = useSelector((state) => state.admin.allUser?.content);
  const rolesData = useSelector((state) => state.admin.allRole);
  const brandData = useSelector((state) => state.admin.allBrand?.content);
  const productTypeData = useSelector(
    (state) => state.admin.allProductType.content
  );
  const sizeData = useSelector((state) => state.admin.allSize?.content);
  const productData = useSelector((state) => state.admin.allProduct?.content);
  const productSizeData = useSelector(
    (state) => state.admin.allProductSize?.content
  );
  const voucherData = useSelector((state) => state.admin.allVoucher?.content);
  const orderData = useSelector((state) => state.admin.allOrder?.content);
  const productOrderData = useSelector(
    (state) => state.admin.allProductOrder?.content
  );
    ...
    if (PaginationData?.length === 0) {
      ...
      let data = PaginationData.map((item) => {
        return {
          ...
          "Ngày mua": formatterDate(item.createDate),
        };
      });
                ...
                >
                  {tableColumns.map((column, columnIndex) => {
                    ...
                    if (gridType === "product-size") {
                      ...
                      return (
                        <td key={columnIndex}>
                          {column.key === "productResponse"
                            ? item[column.key]?.productName
                            : column.key === "sizeResponse"
                  ...
                  {gridType === "order-admin" ? (
                    <td>
                      <Link
                        href={`/admin/order/${orderStatus.orderType}/${item.id}`}
                        className="more"
                      >
                        Xem chi tiết
                      </Link>
