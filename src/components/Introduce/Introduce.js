...
const Introduce = () => {
  ...
  let getProductClassify = async (racket, shoe, shirt) => {
    try {
      ...
      if (
        ...
        setAllData([
          {
            title: racket.categoryName,
            id: racket.categoryId,
            data: resR.data.content,
          },
          {
            title: shoe.categoryName,
            id: shoe.categoryId,
            data: resShoe.data.content,
          },
          {
            title: shirt.categoryName,
            id: shirt.categoryId,
            data: resShirt.data.content,
          },
        ]);
      }
    } catch (err) {
  ...
  let getAllProductTypes = async () => {
    try {
      dispatch(loadingProduct(true));
      let res = await handleGetAllProductTypeService(null, null, null, false);
      if (res && res.code === 0) {
        if (res?.data && res?.data?.content.length > 0) {
          let racket = res?.data?.content.find(
            (productType) => productType.categoryName === "Vợt cầu lông"
          );
          let shoe = res?.data?.content.find(
            (productType) => productType.categoryName === "Giày cầu lông"
          );
          let shirt = res?.data?.content.find(
            (productType) => productType.categoryName === "Áo cầu lông"
          );
          console.log(shoe);
                  ...
                  item.data.map((product, index) => {
                    return (
                      <Grid item xs={3} key={index}>
                        <Link
                          href={`/${convertSlugUrl(
                            product.categoryResponse?.categoryName
                          )}-${product.categoryResponse?.categoryId.toLowerCase()}/${convertSlugUrl(
                            product.productName
                            ...
                            <div style={{ height: "450px" }}>
                              ...
                              >
                                {product.favourite ? (
                                  <FavoriteTwoToneIcon
                                    onClick={() => {
                                      handleClickLike(product.id, "like");
                                    }}
                                  ...
                                  <FavoriteBorderTwoToneIcon
                                    className="favourite-icon"
                                    onClick={() => {
                                      handleClickLike(product.id, "noLike");
                                    }}
                                  />
                                )}
                              </button>
