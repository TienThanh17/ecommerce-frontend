...
import { handleGetCartService } from "../../services/cartService";
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res && res.code === 0) {
        ...
        let useInfor = await handleGetUserAfterLoginService(decoded.sub);
        ...
        // {
        if (useInfor?.code === 0) {
          dispatch(logIn(useInfor.data.user));
          dispatch(updateFavourites(useInfor.data.favoriteProducts));
          let cartId = await handleGetCartService(useInfor.data.user.id);
