...
import "./page.scss";
import { loadingProduct, setCheckUpdateFeedback } from "@/redux-toolkit/productSlice";
...
function ProductDetail({ params }) {
  ...
  const checkUpdateFeedback = useSelector((state) => state.product.checkUpdateFeedback);
  ...
  let getInfoProdut = async () => {
    try {
      ...
      if (res && res.code === 0) {
        setProduct(res?.data);
        setSizeData(res?.data?.sizeResponse);
      } else {
  ...
  useEffect(() => {
    getInfoProdut();
    dispatch(setCheckUpdateFeedback(false));
  }, [params.productId, checkUpdateFeedback]);

  const handleClickSize = (sizeId) => {
    let data = sizeData.map((size) => {
      size.id === sizeId ? (size.selected = true) : (size.selected = false);
      return size;
    });
    let currentSize = data.find((size) => size.id === sizeId);
    setSizeData(data);
