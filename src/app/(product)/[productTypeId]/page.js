...
const Product = ({ params }) => {
  ...
  const productPagination = useSelector(
    (state) => state.product.allProductOfTheProductType.content
  );
  const [paginationData, setPaginationData] = useState([]);
  const brands = useSelector((state) => state.admin.allBrand.content);
                ...
                return (
                  <Grid item xs={4} key={index}>
                    <Link
                      ...
                      )}-${item.categoryResponse.categoryId.toLowerCase()}/${convertSlugUrl(
                        item.productName
                      ...
                      >
                        {item.favourite ? (
                          ...
                          <FavoriteBorderTwoToneIcon
                            className="favourite-icon"
                            onClick={() => {
                              handleClickLike(item.id, "noLike");
                            }}
                          />
                        )}
                      </button>
