...
function VoucherPost() {
  ...
  useEffect(() => {
    if (data) {
      setValue("voucherId", data.id);
      setValue("voucherPrice", currencyFormatter.format(data.discount));
      setValue("timeStart", dayjs(new Date(+data.startTime)));
      setValue("timeEnd", dayjs(new Date(+data.endTime)));
  ...
  const onSubmit = async (submitData) => {
    if (path[3] === "create") {
      ...
      try {
        ...
        formData.append("quantity", submitData.quantity);
        formData.append("startTime", submitData.timeStart.$d.getTime());
        formData.append("endTime", submitData.timeEnd.$d.getTime());
        if(fileImage) formData.append("image", fileImage);
        let res = await handleUpdateVoucherService(data.id, formData);
        if (res && res.code === 0) {
