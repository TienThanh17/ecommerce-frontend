...
const UserPost = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      setValue("address", data.addresses[0]);
      setValue("roleId", data.role);
    }
  }, [data, setValue]);
  ...
  const onSubmit = async (dataSubmit) => {
    if (path[3] === "create") {
      ...
      try {
        dispatch(loadingAdmin(true));
        let res = await handleUpdateUser(data.id, {
          email: dataSubmit.email,
          username: dataSubmit.username,
          password: dataSubmit.password,
          phoneNumber: dataSubmit.phone,
          address: dataSubmit.address,
          role: dataSubmit.roleId,
        });
        if (res && res.code === 0) {
            ...
            >
              {rolesData &&
                ...
                rolesData.map((option) => (
                  ...
                  >
                    {option.name === "ADMIN" ? "Quản trị viên" : "Khách hàng"}
                  </MenuItem>
                ))}
            </TextField>
          )}
