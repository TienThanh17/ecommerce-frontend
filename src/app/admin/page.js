  ...
  useEffect(() => {
    let getAllOrderStatistics = async () => {
      try {
        ...
        if (res && res.code === 0) {
          setRevenueData(res?.data?.orderStatisticResSet);
          setTotalIncome(res?.data?.totalRevenue);
          setTotalOrder(res?.data?.totalOrders);
          setTotalProduct(res?.data?.totalProducts);
          setAllOrderStatus(
            res?.data?.ordersByType.map((status) => {
              if (status.label === "Chờ xác nhận") {
  ...
  useEffect(() => {
    const handleRevenueToMonth = () => {
      ...
      revenueData?.forEach((item) => {
        ...
        const [createDay, createMonth, createYear] = item.createAt.split("-");
        const date = new Date(createYear, createMonth - 1, createDay);
