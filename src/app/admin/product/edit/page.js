...
const ProductPost = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      setValue("brandID", data.brandResponse?.brandId);
      setValue("productType", data.categoryResponse?.categoryId);
    ...
    if (path[3] === "create") {
      if (!imageValue || !fileImage) {
        ...
        try {
          ...
          formData.append("categoryId", submitData.productType);
          formData.append("productName", submitData.productName);
      ...
      try {
        ...
        let res = await handleUpdateProductService(data.id, formData);
        ...
        if (err?.response?.data?.code === 17) {
          toast.error("Sản phẩm đã tồn tại");
        } else if (err?.response?.data?.code === 18) {
