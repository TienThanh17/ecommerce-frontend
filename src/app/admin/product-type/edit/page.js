...
const ProductTypePost = () => {
  ...
  useEffect(() => {
    if (data) {
      setValue("typeID", data.categoryId);
      setValue("typeName", data.categoryName);
    }
  }, [data, setValue]);
  ...
  const onSubmit = async (submitData) => {
    if (path[3] === "create") {
      ...
      try {
        dispatch(loadingAdmin(true));
        let res = await handleUpdateProductTypeService(data.id, {
          categoryId: submitData.typeID,
          categoryName: submitData.typeName,
        });
        if (res && res.code === 0) {
