...
function Profile() {
  ...
  const roleId = useSelector((state) => state.user.userInfo?.role);
    ...
    try {
      ...
      if (res && res.code === 0) {
        ...
        res?.data?.addresses?.length > 0
          ? setValue("address", res?.data?.addresses[0])
          : setValue("address", "");
        if (res?.data?.dateOfBirth)
          setValue("birth", dayjs(res?.data?.dateOfBirth));
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      formData.append("dateOfBirth", data.birth.toISOString().split('T')[0]);

      if (fileAvatar && fileAvatar !== oldFileAvatar) {
        formData.append("avatar", fileAvatar);
      }
      let res = await handleUpdateUser(userId, formData);
      if (res && res.code === 0) {
